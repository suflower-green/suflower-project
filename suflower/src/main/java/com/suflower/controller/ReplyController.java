...
public class ReplyController {
	...
	// 특정 게시물의 댓글목록 확인
	@GetMapping(value="/pages/{boardNo}/{pageNum}", produces= {MediaType.APPLICATION_XML_VALUE,MediaType.APPLICATION_JSON_UTF8_VALUE})
	...
	public ResponseEntity<String> remove(@PathVariable("replyNo")Long replyNo){
		...
		return rservice.remove(replyNo)==1 ? new ResponseEntity<>("success",HttpStatus.OK) : new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
	...
	public ResponseEntity<String> modify(@RequestBody ReplyDTO dto,@PathVariable("replyNo")Long replyNo){
		...
		return rservice.modify(dto)==1 ? new ResponseEntity<>("success",HttpStatus.OK) : new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
	}
